...
function AnnouncementCard({
  ...
  return (
    <div style={{
      ...
      <div style={s.cardHead}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            {ann.pinned && (
              <span style={s.pinnedPill}>📌 Pinned</span>
            )}
          </div>
          <div style={s.dateLine}>{timeAgo(ann.created_at)}</div>
        </div>
      ...
      <div style={s.divider} />
      <button style={s.addCommentBtn}>💬  Add comment</button>
    </div>
  )
}
...
export default function AnnouncementsPage() {
  ...
  const postMutation = useMutation({
    ...
    onError: (err: Error) => {
      toast({ title: "Failed to post", description: err.message || "Unknown error", variant: "destructive" })
    }
  })
      ...
      <div style={s.page}>
        ...
        <div style={s.pageHeader}>
          <div>
            ...
            <p style={s.pageSub}>
              {isTeacher ? "Post updates visible to all your students" : "Updates from your teachers"}
            </p>
          </div>
        ...
        {announcements.length === 0 && (
          <div style={s.empty}>
            ...
            <p style={{ color: "#9ca3af", marginTop: 12, fontSize: 14 }}>No announcements yet</p>
            {isTeacher && <p style={{ color: "#d1d5db", fontSize: 13 }}>Post something above to notify your students</p>}
          </div>
        )}

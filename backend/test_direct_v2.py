...
if not perf:
    print(f"[ERR] No performance data found for {student_id}")
    db.close()
    sys.exit(1)

print(f"[OK] Found performance data:")
print(
...
print(f"\n[REQ] Request data created")

# Process analytics
try:
    response = process_student_analytics(request_data)
    response_dict = response.model_dump()
    print(f"\n[OK] Analytics response generated successfully!")
    ...
    print(f"\n[ERR] Error processing analytics: {e}")
    import traceback
    traceback.print_exc()
